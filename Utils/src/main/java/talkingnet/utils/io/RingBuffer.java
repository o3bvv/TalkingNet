...
public final class RingBuffer<T> {
    ...
    public synchronized T peek() throws InterruptedException {
         ...
         * tricky to compute. We know that there are unconsumedElements elements
         * waiting to be read, and they're contiguously before the write
         * position. However, the buffer wraps around itself, and so we can't
         * just do a naive subtraction; that might end up giving us a negative
         * index. To avoid this, we'll use a clever trick in which we'll add to
         * the index the capacity minus the distance. This value must be
         * positive, since the distance is never greater than the capacity, and
         * if we then wrap this value around using the modulus operator we'll
         * end up with a valid index. All of this machinery works because
         *
